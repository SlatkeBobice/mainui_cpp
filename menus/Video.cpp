...
void UI_Video_Menu( void )
{
	if( !menu_video->WasInit() )
	{
		...
		menu_video->AddButton( L( "Графон" ), L( NULL ), PC_VID_OPT, UI_VidOptions_Menu, QMF_NOTIFY );
		menu_video->AddButton( L( "Разрешения" ), L( NULL ), PC_VID_MODES, UI_VidModes_Menu, QMF_NOTIFY );
		menu_video->AddButton( L( "Применить" ), L( NULL ), PC_DONE, VoidCb( &CMenuFramework::Hide ), QMF_NOTIFY );
	}
