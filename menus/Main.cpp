...
{
	dialog.SetMessage( L( "Э, а играть кто будет?" ) );
...
void CMenuMain::_Init( void )
{
	...
	// console
	console.SetNameAndStatus( L( "GameUI_Console" ), L( NULL ) );
	...
	quit.SetNameAndStatus( L( "GameUI_GameMenu_Quit" ), L( NULL ) );
...
void CMenuMain::VidInit( bool connected )
{
	...
	{
		saveRestore.SetNameAndStatus( L( "Save\\Load Game" ), L( NULL ) );
		saveRestore.SetPicture( PC_SAVE_LOAD_GAME );
		saveRestore.onReleased = UI_SaveLoad_Menu;
	}
	else
	{
		saveRestore.SetNameAndStatus( L( "GameUI_LoadGame" ), L( NULL ) );
